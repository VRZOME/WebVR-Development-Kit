...
var vrScreenHeight = window.innerHeight;
var vrScreenWidth  = window.innerWidth;
/**
...
vr.HmdInfo = function(opt_values) {
  ...
  this.resolutionHorz = opt_values ?
      parseFloat(opt_values[o++]) : vrScreenWidth;

  //this.resolutionHorz = window.innerWidth;
  ...
  this.resolutionVert = opt_values ?
      parseFloat(opt_values[o++]) : vrScreenHeight;

  //this.resolutionVert = window.innerHeight;
  ...
  this.screenSizeHorz = opt_values ?
      parseFloat(opt_values[o++]) : 0.12096;
  ...
  this.screenSizeVert = opt_values ?
      parseFloat(opt_values[o++]) : 0.06804;
  ...
  this.screenCenterVert = opt_values ?
      parseFloat(opt_values[o++]) : vrScreenHeight / 2;

  //this.screenCenterVert = window.innerHeight / 2;
...
vr.StereoRenderer.prototype.initialize_ = function() {
  ...
  // hawksley: I've had issues with this detecting the wrong screen size,
  // the two lines below are a hack to fix that if that starts
  // occurring again.
  // info.resolutionHorz = 1280;
  // info.resolutionVert = 800;
...
vr.StereoRenderer.prototype.setWidthHeight = function(key) {
    if(key == 0)
    {
	    vrScreenHeight = window.innerHeight;
	    vrScreenWidth  = window.innerWidth;
    }	
	if(key == 1)
    {
	    vrScreenHeight = 2560;
...
vr.StereoRenderer.prototype.render = function(vrstate, callback, opt_scope) {
  ...
  if (nowPresent == true && vr.isFullScreen() == false)
  {
      vrstate.hmd.present = false;console.log("C");
  }
  if (nowPresent == false && vr.isFullScreen() == true)
  {
        vrstate.hmd.present = true;console.log("D");
  }		
  ...
  if (nowPresent != this.hmdPresent_) {
    if (nowPresent) {
      ...
      vrScreenWidth = 2560;
	    vrScreenHeight = 1440;
      this.hmdInfo_ = new vr.HmdInfo();console.log("A");
    } else {
      this.hmdPresent_ = false;
      // Disconnected. Reset to defaults.
      vrScreenHeight = window.innerHeight;
      vrScreenWidth  = window.innerWidth;
      this.hmdInfo_ = new vr.HmdInfo();console.log("B");
    }
    this.initialize_();
  }
